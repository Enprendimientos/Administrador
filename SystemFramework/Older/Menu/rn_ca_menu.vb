...
#Region "Metodos asociados a Administracion"
        ...
        Public Function Insert(ByVal objDatos As cm_ca_menu) As Boolean
            ...
            ApplicationVerifications.CheckCondition(objDatos.Tables(cm_ca_menu.TABLA).Rows.Count > 0, "Para poder insertar debe tener al menos un registro en la objDatos", ApplicationVerifications.LineNumber)

            With objDatos.Tables(cm_ca_menu.TABLA).Rows(0)
                Dim dr As DataRow
                Try
                    ' revisa cada una de las filas de la tabla
                    For Each dr In objDatos.Tables(cm_ca_menu.TABLA).Rows
                        ' valida la fila 
                        result = Validate(dr)
                    Next
        ...
        Public Function Update(ByVal objDatos As cm_ca_menu) As Boolean
            Try
                ...
                ApplicationVerifications.CheckCondition(objDatos.Tables(cm_ca_menu.TABLA).Rows.Count = 1, "El parametro objDatos debe contener una fila", ApplicationVerifications.LineNumber)
                With objDatos.Tables(cm_ca_menu.TABLA).Rows(0)
                    result = Validate(objDatos.Tables(cm_ca_menu.TABLA).Rows(0))
                    If result Then
                        myTrans.BeginTransaction()
                        With New ad_ca_menu(myTrans)
        ...
        Private Shadows Function Validate(ByVal ObjReg As DataRow) As Boolean
            ...
            Try
                ' valida los largo
                isValid = MyBase.ValidaIntegridad(New cm_ca_menu, ObjReg)
